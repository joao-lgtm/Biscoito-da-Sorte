...
let cookieBreack = false;
let button = document.createElement("button");

button.setAttribute("onClick", "reload()");
button.setAttribute("id", "try-again");
button.textContent = "Abrir outro biscoito";


const luckphrases = () => {
    numberRandom = Math.floor(Math.random() * (61 - 0) + 1 - 1)
    return phrases[numberRandom]
}
...
let openCookie = () => {
    if (cookieBreack != true) {
        cookie.removeAttribute("id")
        cookie.setAttribute("src", "assets/opened-cookie.svg");
        title.innerText = "Aqui está a sua sorte de hoje:"
        p.setAttribute("id" , "phrase")
        p.textContent = luckphrases();
